...
*   **`california_feature_selection_comparison.png`** — 展示 11 種篩選方案在特徵數量 $k \in [1, 10]$ 下的 RMSE 與 $R^2$ 走勢變化，以及前 10 大特徵排名矩陣對照表。
*   **`allinone.png`** — 加州房價綜合資料探索 (EDA) 看板。
*   **單一圖表檔案**：
    *   `california_correlation_heatmap.png` — 相關性熱力矩陣。
    *   `income_vs_house_value.png` — 收入與房價關係散點圖。
    *   `ocean_proximity_vs_house_value.png` — 地理靠海類別對房價的影響。
    *   `california_feature_importance.png` — 隨機森林特徵重要度權重排名。

---

## 📊 視覺化成果展示 (Visualization Results)

### 1. 11 種特徵篩選方案性能對比與特徵排名矩陣
本專案評估了 11 種不同的特徵選取機制在 $k \in [1, 10]$ 時的預測性能（RMSE 與 $R^2$），並輸出完整的特徵排名表：

![California Feature Selection Comparison](california_feature_selection_comparison.png)

### 2. 資料探索 (EDA) 綜合分析看板
下圖展示了加州房價資料集中的重要地理、人口與收入分布關係：

![California Housing EDA Dashboard](allinone.png)
